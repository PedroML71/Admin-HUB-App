...
const FuncionarioGraph = ({ unidadeId, role }) => {
  const { data: response, isLoading } = useGetUsuarioSignupHistoryQuery(
    role === "Admin"
      ? { params: { role: "Funcionario" } }
      : { params: { role: "Funcionario" }, unidadeId: unidadeId }
  );
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-center gap-4 p-4 border-b border-b-primary-grey">
            <WrenchIcon className="w-16 text-dark-blue" />

            <p className="text-2xl text-dark-blue">Funcionarios</p>
          </div>
...
FuncionarioGraph.propTypes = {
  unidadeId: PropTypes.string,
  role: PropTypes.string.isRequired,
};

export default FuncionarioGraph;
