...
const Funcionarios = () => {
  ...
  const { data: response } = useGetAllUsuariosQuery(
    auth?.user?.role === "Funcionario"
      ? {
          params: { page, nome, limit: "8", role: "Funcionario" },
  ...
  const searchHandler = (nome) => {
    if (!nome) {
      ...
      if (funcionarios.length !== 0 && searchMode === false) {
        setFuncionarios([]);
      }
  ...
  return (
    <section>
      <Title name={"Funcionarios"} spacingStyle={"pt-12 pb-8"} />
      ...
      <Menu>
        ...
        <Modal visible={modal} onClose={closeCreateModalHandler}>
          <FuncionarioForm closeCreateModalHandler={closeCreateModalHandler} />
        </Modal>
      </Menu>

      <InfiniteScroll
        dataLength={funcionarios.length}
        next={fetchData}
        hasMore={hasMore}
        loader={<Loading />}
        endMessage={
      ...
      >
        <FuncionarioLista lista={funcionarios} />
      </InfiniteScroll>
    </section>
  );
};
